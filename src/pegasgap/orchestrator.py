...
async def run_pair(
    ...
    600 с): если внутренние таймауты не сработали (зависший браузер), `search()`
    отменяется и не подвешивает прогон.

    Значение должно с запасом покрывать САМОЕ ДОЛГОЕ честное чтение, иначе предохранитель
    начинает срабатывать на здоровых прогонах. Прежние 180 с были подобраны, когда выдачу
    Турвизора читали на десять страниц; после перехода на полное чтение Турция стала
    занимать около 200 с, три таких подряд выглядели как «легла площадка», и обход
    останавливал сам себя. Затем в 420 с перестала укладываться Россия при параллельном
    обходе: у Слетать там 800+ отелей в выдаче, обе площадки читаются дольше всего, а
    соседние параллельные кейсы отнимают полосу. Отсюда 600 с — запас над худшим честным
    случаем; настоящая смерть площадки всё равно ловится, просто на три минуты позже.
    ...
    timeout_s = (provider_timeout_s if provider_timeout_s is not None
                 else float(os.environ.get("PEGASGAP_PROVIDER_TIMEOUT_S") or 600))
