...
class GapKind(StrEnum):
    ...
    FULL = "full"                      # эталон: предложения есть; проверяемая: ни одного
    NOT_RESPONDING = "not_responding"  # проверяемая площадка: оператор не ответил
    HOTEL = "hotel"                    # отель есть на эталоне, у оператора на проверяемой нет
    PRICE = "price"                    # есть на обеих, цена расходится сверх порога
    REVERSE = "reverse"                # есть на проверяемой, нет на эталоне
    ...
    def hint(self) -> str:
        ...
        return {
            ...
            GapKind.REVERSE: "обычно шум эталона; если массово — проверить матчинг отелей",
        }[self]
...
class ScanResult(BaseModel):
    ...
    # витрина эталона показывает по оператору выборку, а не весь каталог) — не поломка,
    # и если считать его поломкой, инструмент будет объявлять недостоверным каждый прогон.
