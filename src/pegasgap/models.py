...
class GapKind(StrEnum):
    ...
    @property
    def title(self) -> str:
        # Названия говорят, ЧТО случилось и ГДЕ. «Отельный пропуск» понятно только
        # автору; и раз ни одна площадка больше не считается эталоном, в названии должно
        # стоять имя той, где предложений не оказалось.
        return {
            GapKind.FULL: "Нет на Слетать совсем",
            GapKind.NOT_RESPONDING: "Оператор не ответил на Слетать",
            GapKind.HOTEL: "Нет на Слетать по отелю",
            GapKind.PRICE: "Цена расходится",
            GapKind.REVERSE: "Нет на Турвизоре",
        }[self]
