...
**Почему по странам, а не по парам «город → страна».** Пара была бы точнее, но весь
замер держится на дешёвом счётчике `RowsCount`, а он городов НЕ различает: по Турции
Петербург, Екатеринбург, Казань и Новосибирск дают ровно 272 — одно и то же число.
Раньше отсюда делали вывод, что шлюз игнорирует `cityFromId`; это неверно. Полные выдачи
на тех же параметрах разные — 780, 770, 649 и 724 отеля, отпечатки состава и цен не
совпадают. Не различает именно счётчик. Значит, ранжировать по паре можно, только вытянув
выдачу целиком, — а это уже не дешёвый замер, а полный поиск на каждую пару.
