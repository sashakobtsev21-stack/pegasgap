...
#
# Порог растёт вместе с параллельностью: при N одновременных кейсах одна неудачная
# ПАЧКА — это N сбоев разом, а очередь идёт направлениями подряд, так что пачка часто
# целиком из одного тяжёлого направления. Живой случай: три кейса «Москва—Россия,
# отели» в одной пачке не уложились в таймаут — серия из 3 набралась мгновенно, и обход
# остановился при живых площадках. Порог «параллельность + 2» переживает одну плохую
# пачку, но настоящая смерть площадки (валится всё подряд) останавливает обход в
# пределах второй.
MAX_CONSECUTIVE_FAILURES = (int(os.environ.get("PEGASGAP_MAX_FAILURES") or 0)
                            or max(3, CONCURRENCY + 2))
