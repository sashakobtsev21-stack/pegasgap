...
# дало 587 находок из 641, полностью похоронив настоящие: объёмы программ у площадок
# разные, и каждый лишний отель на нашей стороне становился «находкой».
...
def detect(
    ...
    if not REPORT_REVERSE:
        ...
        result.notes.append(
            ...
            f"объёмы несопоставимы. Обратные пропуски скрыты как заведомый шум; "
            f"отельные пропуски ищутся среди того, что показал эталон")
    else:
        gaps += [
            HotelGap(
