...
def detect(
    ...
    if not REPORT_REVERSE:
        ...
        if reference is not None and reference.truncated:
            # Недочитанный листинг больше не выключает сторону: «не в листинге» здесь
            # недоказуемо, но у кандидатов есть прижатые пробы (reversecheck), и именно
            # они решат судьбу каждого. Без проб такие кандидаты будут сняты там же.
            result.notes.append(
                f"выдача Турвизора прочитана не до конца ({len(ref_hotels)} отел(ей)) — "
                f"кандидаты «нет на Турвизоре» будут удержаны только прижатыми пробами")
        elif _coverage_is_lopsided(len(ref_hotels), len(chk_hotels)):
            # Обе выдачи полные — разница объёмов и есть результат, а не помеха. Но
            # сказать о ней стоит, иначе список на сотни строк читается как авария.
            result.notes.append(
